...
describe("Vesting Contract", function () {
  ...
  it("Should not allow non-backend to revoke DEPOSITOR_ROLE from some other address", async function () {
    ...
    await contractVesting.connect(backend).grantDepositerRole(newDepositor);
    expect(await contractVesting.hasRole(DEPOSITOR_ROLE, newDepositor)).to.equal(true);
    await expect(contractVesting.connect(account1).revokeDepositerRole(newDepositor)).to.be.revertedWith(
  ...
  it("Should allow DEPOSITOR_ROLE to create new deposit for some other address", async function () {
    ...
    expect(await contractVesting.connect(backend).deposit(beneficiary, amount, releaseTime))
      .to.emit(contractVesting, "VestingDeposit")
      .withArgs(backend.address, beneficiary, currentVestingIndex, amount, releaseTime);
  ...
  it("Should not allow non-DEPOSITOR_ROLE to create new deposit for some other address", async function () {
    ...
    /// @dev releaseTime is 1 day from now.
    const releaseTime = (await hre.ethers.provider.getBlock(currentBlockNumber)).timestamp + 86400;
    await expect(contractVesting.connect(account1).deposit(beneficiary, amount, releaseTime)).to.be.revertedWith(
  ...
  it("Should allow DEPOSITOR_ROLE to create new deposits in batch for some other addresses", async function () {
    ...
    const tx = contractVesting
      .connect(backend)
      .depositInBatch([beneficiary1, beneficiary2, beneficiary3], amounts, releaseTimes);
    await Promise.all([
      expect(tx)
        .to.emit(contractVesting, "VestingDeposit")
        .withArgs(backend.address, beneficiary1, currentVestingIndex, amounts[0], releaseTimes[0]),
      expect(tx)
        .to.emit(contractVesting, "VestingDeposit")
        .withArgs(backend.address, beneficiary2, currentVestingIndex + 1, amounts[1], releaseTimes[1]),
      expect(tx)
        .to.emit(contractVesting, "VestingDeposit")
        .withArgs(backend.address, beneficiary3, currentVestingIndex + 2, amounts[2], releaseTimes[2]),
    ]);
    // Check balance of backend
    expect(await contractUDAO.balanceOf(backend.address)).to.be.equal(
      currentBalanceOfBackend.sub(amounts[0]).sub(amounts[1]).sub(amounts[2])
    );
    // Check balance of contractVesting
    expect(await contractUDAO.balanceOf(contractVesting.address)).to.be.equal(
  ...
  it("Should allow a beneficiary to withdraw his vested amount if vesting period is over", async function () {
    ...
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex))
      .to.emit(contractVesting, "VestingWithdrawal")
      .withArgs(beneficiary, currentVestingIndex, amount);
  ...
  it("Should not allow a beneficiary to withdraw his vested amount if vesting period is not over", async function () {
    ...
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex)).to.be.revertedWith(
  ...
  it("Should allow a beneficiary to withdraw his vested amount if vesting period is over and he has multiple vesting deposits", async function () {
    ...
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex))
      .to.emit(contractVesting, "VestingWithdrawal")
      .withArgs(beneficiary, currentVestingIndex, amount1);
    // Check balance of beneficiary after withdraw
    expect(await contractUDAO.balanceOf(account1.address)).to.be.equal(amount1);
    // Beneficiary should be able to withdraw amount2
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex + 1))
      .to.emit(contractVesting, "VestingWithdrawal")
      .withArgs(beneficiary, currentVestingIndex + 1, amount2);
    // Check balance of beneficiary after withdraw
    ...
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex + 2))
      .to.emit(contractVesting, "VestingWithdrawal")
      .withArgs(beneficiary, currentVestingIndex + 2, amount3);
    // Check balance of beneficiary after withdraw
  ...
  it("Should allow a beneficiary to withdraw his vesting that vested period is over and not allow to withdraw his vesting that vested period is not over", async function () {
    ...
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex + 1)).to.be.revertedWith(
      "Your vesting period is not over yet"
    );
    // Check balance of beneficiary after withdraw
    expect(await contractUDAO.balanceOf(account1.address)).to.be.equal(amount1);
    // Beneficiary should not be able to withdraw amount3
    await expect(contractVesting.connect(account1).withdraw(currentVestingIndex + 2)).to.be.revertedWith(
      "Your vesting period is not over yet"
    );
    // Check balance of beneficiary after withdraw
    expect(await contractUDAO.balanceOf(account1.address)).to.be.equal(amount1);
  ...
  it("Should not allow a user to use an existing vesting index to withdraw someone else's vesting", async function () {
    ...
    // Check balance of backend
    expect(await contractUDAO.balanceOf(backend.address)).to.be.equal(
      currentBalanceOfBackend.sub(amount1).sub(amount2)
    );
    // Check balance of contractVesting
    ...
    await expect(contractVesting.connect(account2).withdraw(currentVestingIndex + 1))
      .to.emit(contractVesting, "VestingWithdrawal")
      .withArgs(beneficiary2, currentVestingIndex + 1, amount2);
  ...
  it("Should not allow to deposit if DEPOSITOR_ROLE doesn't have enough tokens", async function () {
    ...
    /// @dev releaseTime is 1 day from now
    const releaseTime = (await hre.ethers.provider.getBlock(currentBlockNumber)).timestamp + 86400;
    await expect(contractVesting.connect(backend).deposit(beneficiary, amount, releaseTime)).to.be.revertedWith(
